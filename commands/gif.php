...
$commands['gif'] = function(&$conn, $event, $params) {
	if(!empty($params[0])) {
		...
		$url = "http://api.giphy.com/v1/gifs/search?q=" . urlencode($param_str) . "&limit=1&api_key=dc6zaTOxFJmzC";
		$result = Unirest::get($url);
		if(!empty($result->body->data) && !empty($result->body->data[0]->images)) {
			$img = $result->body->data[0]->images->original;
